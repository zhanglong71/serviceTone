...
unsigned char Sja_1000_Init(unsigned char intMode)
{
 ...
 s=BCAN_SET_OBJECT(0x12,0x34,0x16,0x00,0x00,0xff,0xff,0xf0);//屏蔽寄存器，都设为无关，接收所有报文 
                                                            //当屏蔽位为1，不滤波，0就滤波必须相等
...
void canSendRtr(unsigned char pAcr0,unsigned char pAcr1)
{
	unsigned char temptt;
	CntCanTxTimeout=0;
	flag_CanTxTimeout=0;
